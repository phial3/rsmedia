...
#[repr(transparent)]
#[derive(Copy, Clone)]
pub struct ChannelLayout(pub ffi::AVChannelLayout);
    ...
    fn eq(&self, other: &Self) -> bool {
        unsafe {
            ffi::av_channel_layout_compare(
                &self.0 as *const ffi::AVChannelLayout,
                &other.0 as *const ffi::AVChannelLayout,
            ) == 0
        }
    }
}
...
macro_rules! define_layout {
    ($name:ident, $nb:expr, $mask:expr) => {
        pub const $name: ChannelLayout = ChannelLayout(ffi::AVChannelLayout {
            order: ffi::AV_CHANNEL_ORDER_NATIVE,
            nb_channels: $nb,
            u: ffi::AVChannelLayout__bindgen_ty_1 { mask: $mask },
...
impl ChannelLayout {
    ...
    pub fn default(number: i32) -> ChannelLayout {
        unsafe {
            ...
            ffi::av_channel_layout_default(&mut channel_layout, number);
    ...
    pub fn is_empty(&self) -> bool {
        self.0.order == ffi::AV_CHANNEL_ORDER_UNSPEC
    }
}

impl From<ffi::AVChannelLayout> for ChannelLayout {
    fn from(value: ffi::AVChannelLayout) -> Self {
        Self(value)
    }
}

impl From<ChannelLayout> for ffi::AVChannelLayout {
