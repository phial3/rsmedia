//! This module provides functions to compare H.264 frames to images.
...
//! assert_h264_frame("tests/initial-grid.png", &actual, 0.9);
//! ```
...
//! assert_image("tests/dog1.png", &actual, 0.9);
//! ```
//!
...
pub(crate) fn assert_image_impl<P: AsRef<std::path::Path>>(
    ...
    let expected = match image::ImageReader::open(path) {
    ...
    if mode == Mode::StoreArtifact || (mode == Mode::StoreArtifactOnMismatch && image_mismatch) {
        ...
        if let Some(parent) = artifact_path.parent() {
            std::fs::create_dir_all(parent).expect("unable to create artifact directory");
        }
...
pub(crate) fn h264_frame_to_image(data: &[u8]) -> Result<image::DynamicImage> {
    ...
    let mut temp_file = std::fs::File::create(&temp_file_name).expect("unable to create temp file");
    temp_file
        .write_all(data)
        .expect("unable to write to temp file");

    // Create a decoder for the H.264 format
    let ictx = ffmpeg::format::input(&temp_file_name).map_err(|e| e)?;
    let input = ictx
        .streams()
    ...
    let Some(raw) = image::RgbImage::from_raw(
        video_frame.width(),
        video_frame.height(),
        video_frame.data(0).to_vec(),
    ) else {
        return Err("the container was not big enough as per: https://docs.rs/image/latest/image/struct.ImageBuffer.html#method.from_raw".to_string().into());
    };
